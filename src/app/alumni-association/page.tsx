...
async function getSiteSettings(): Promise<SiteSettings> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || 'https://serveraasc.veetusaapadu.in'}/api/site`,
    { cache: 'force-cache' }
  );
...
async function getAlumni() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || 'https://serveraasc.veetusaapadu.in'}/api/alumni`,
    { cache: 'force-cache' }
  );
